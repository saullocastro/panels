...
def fkCSB11_dmg(double kt, double dsb, object p1, int size, int row0, int col0, 
            int no_x_gauss, int no_y_gauss, kw_tsl):
    r"""
    ...
    with nogil:
        ...
        for ptx in range(no_x_gauss):
            for pty in range(no_y_gauss):
                ...
                c = -1
                for i1 in range(m1):
                    f1Au = f(i1, xi, x1u, x1ur, x2u, x2ur)
                    # f1Auxi = fp(i1, xi, x1u, x1ur, x2u, x2ur)
                    f1Av = f(i1, xi, x1v, x1vr, x2v, x2vr)
                    # f1Avxi = fp(i1, xi, x1v, x1vr, x2v, x2vr)
                    f1Aw = f(i1, xi, x1w, x1wr, x2w, x2wr)
                    f1Awxi = fp(i1, xi, x1w, x1wr, x2w, x2wr)
                    # f1Awxixi = fpp(i1, xi, x1w, x1wr, x2w, x2wr)
                    
                    for k1 in range(m1):
                        f1Bu = f(k1, xi, x1u, x1ur, x2u, x2ur)
                        # f1Buxi = fp(k1, xi, x1u, x1ur, x2u, x2ur)
                        f1Bv = f(k1, xi, x1v, x1vr, x2v, x2vr)
                        # f1Bvxi = fp(k1, xi, x1v, x1vr, x2v, x2vr)
                        f1Bw = f(k1, xi, x1w, x1wr, x2w, x2wr)
                        f1Bwxi = fp(k1, xi, x1w, x1wr, x2w, x2wr)
                        # f1Bwxixi = fpp(k1, xi, x1w, x1wr, x2w, x2wr)
                        
                        for j1 in range(n1):
                            g1Au = f(j1, eta, y1u, y1ur, y2u, y2ur)
                            # g1Aueta = fp(j1, eta, y1u, y1ur, y2u, y2ur)
                            g1Av = f(j1, eta, y1v, y1vr, y2v, y2vr)
                            # g1Aveta = fp(j1, eta, y1v, y1vr, y2v, y2vr)
                            g1Aw = f(j1, eta, y1w, y1wr, y2w, y2wr)
                            g1Aweta = fp(j1, eta, y1w, y1wr, y2w, y2wr)
                            # g1Awetaeta = fpp(j1, eta, y1w, y1wr, y2w, y2wr)
                                    
                            for l1 in range(n1):
                                g1Bu = f(l1, eta, y1u, y1ur, y2u, y2ur)
                                # g1Bueta = fp(l1, eta, y1u, y1ur, y2u, y2ur)
                                g1Bv = f(l1, eta, y1v, y1vr, y2v, y2vr)
                                # g1Bveta = fp(l1, eta, y1v, y1vr, y2v, y2vr)
                                g1Bw = f(l1, eta, y1w, y1wr, y2w, y2wr)
                                g1Bweta = fp(l1, eta, y1w, y1wr, y2w, y2wr)
                                # g1Bwetaeta = fpp(l1, eta, y1w, y1wr, y2w, y2wr)
        ...
        for ptx in range(no_x_gauss):
            for pty in range(no_y_gauss):
                ...
                for i1 in range(m1):
                    ...
                    for k2 in range(m2):
                        f2Bu = f(k2, xi, x1u, x1ur, x2u, x2ur)
                        # f2Buxi = fp(k2, xi, x1u, x1ur, x2u, x2ur)
                        f2Bv = f(k2, xi, x1v, x1vr, x2v, x2vr)
                        # f2Bvxi = fp(k2, xi, x1v, x1vr, x2v, x2vr)
                        f2Bw = f(k2, xi, x1w, x1wr, x2w, x2wr)
                        # f2Bwxi = fp(k2, xi, x1w, x1wr, x2w, x2wr)
                        # f2Bwxixi = fpp(k2, xi, x1w, x1wr, x2w, x2wr)
                        ...
                        for j1 in range(n1):
                            ...
                            for l2 in range(n2):
                                g2Bu = f(l2, eta, y1u, y1ur, y2u, y2ur)
                                # g2Bueta = fp(l2, eta, y1u, y1ur, y2u, y2ur)
                                g2Bv = f(l2, eta, y1v, y1vr, y2v, y2vr)
                                # g2Bveta = fp(l2, eta, y1v, y1vr, y2v, y2vr)
                                g2Bw = f(l2, eta, y1w, y1wr, y2w, y2wr)
                                # g2Bweta = fp(l2, eta, y1w, y1wr, y2w, y2wr)
                                # g2Bwetaeta = fpp(l2, eta, y1w, y1wr, y2w, y2wr)
    ...
    with nogil:
        ...
        for ptx in range(no_x_gauss):
            for pty in range(no_y_gauss):
                ...
                c = -1
                for i2 in range(m2):
                    f2Au = f(i2, xi, x1u, x1ur, x2u, x2ur)
                    # f2Auxi = fp(i2, xi, x1u, x1ur, x2u, x2ur)
                    f2Av = f(i2, xi, x1v, x1vr, x2v, x2vr)
                    # f2Avxi = fp(i2, xi, x1v, x1vr, x2v, x2vr)
                    f2Aw = f(i2, xi, x1w, x1wr, x2w, x2wr)
                    # f2Awxi = fp(i2, xi, x1w, x1wr, x2w, x2wr)
                    # f2Awxixi = fpp(i2, xi, x1w, x1wr, x2w, x2wr)
                    ...
                    for k2 in range(m2):
                        ...
                        for j2 in range(n2):
                            g2Au = f(j2, eta, y1u, y1ur, y2u, y2ur)
                            # g2Aueta = fp(j2, eta, y1u, y1ur, y2u, y2ur)
                            g2Av = f(j2, eta, y1v, y1vr, y2v, y2vr)
                            # g2Aveta = fp(j2, eta, y1v, y1vr, y2v, y2vr)
                            g2Aw = f(j2, eta, y1w, y1wr, y2w, y2wr)
                            # g2Aweta = fp(j2, eta, y1w, y1wr, y2w, y2wr)
                            # g2Awetaeta = fpp(j2, eta, y1w, y1wr, y2w, y2wr)
