...
if __name__ == "__main__":
    ...
    if local_run:
        ...
        # p3_70_25_m18_8_ki1e5_tauo67_nx60_ny30_wpts30_G1c112
            dmg_index, del_d, kw_tsl, force_intgn, c_all = test_dcb_damage_prop_fcrack(phy_dim=[3,65,25,48], 
                no_terms=[4,4,4,4], name=['test_nokcrack',''], k_i=1e4, tau_o=87, no_x_gauss=50, 
        ...
        if True:
            dmg_index, del_d, kw_tsl, force_intgn, c_all = test_dcb_damage_prop_kcrack_complete(phy_dim=[3,65,25,48], 
                no_terms=[4,4,4,4], name=['test_kcrcompl',''], k_i=1e4, tau_o=87, no_x_gauss=5, 
                no_y_gauss=2, w_iter_info=[6,8], G1c=1.12)
    ...
    if not local_run:               
        ...
        foldername = 'Theo_mITR_p3_90_255_566_k5e4_t137_nx60y30_w70_G141'
        
        ftn_arg_1 = [([3,90,25.5,56.6],[m_conn,m_conn,m_bend,m_bend],5e4,137,60,30,[70,12],1.41, 
                      [f'p3_90_255_566_m{m_conn:.0f}_{m_conn:.0f}_{m_bend:.0f}_{m_bend:.0f}k5e4_t137_nx60y30_w70_G141', foldername]) 
                    for m_conn in [6,8,10,12,15] for m_bend in [6,8,10,12]]
        ftn_arg_2 = [([3,90,25.5,56.6],[m_conn,m_conn-2,m_bend,m_bend-2],5e4,137,60,30,[70,12],1.41, 
                      [f'p3_90_255_566_m{m_conn:.0f}_{m_conn-2:.0f}_{m_bend:.0f}_{m_bend-2:.0f}k5e4_t137_nx60y30_w70_G141', foldername]) 
                    for m_conn in [8,10,12,15] for m_bend in [8,10,12]]
        ...
        with Pool(processes=32) as pool: # using 20 cpus on the cluster
