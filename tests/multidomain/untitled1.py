# -*- coding: utf-8 -*-
"""
Created on Sat Sep 14 14:42:19 2024
...
def test_dcb_damage_pristine(phy_dim, no_terms, k_i=None, tau_o=None, no_x_gauss=None, no_y_gauss=None, w_iter_info=None, G1c=None, name=''):
    ...
    if BC == 'bot_end_fixed':
        ...
        bot1.x1u = 1 ; bot1.x1ur = 1 ; bot1.x2u = bot_t ; bot1.x2ur = 1
        bot1.x1v = 1 ; bot1.x1vr = 1 ; bot1.x2v = bot_t ; bot1.x2vr = 1
    ...
    for wp in w_iter: 
        ...
        if True:
            ...
            if local_run:
                ...
                plt.plot(force_intgn_dmg[:disp_iter_no+1, 0], force_intgn_dmg[:disp_iter_no+1, 1], label='Area Int')
...
def test_dcb_damage_pristine_mult_pan(phy_dim, no_terms, k_i=None, tau_o=None, no_x_gauss=None, no_y_gauss=None, w_iter_info=None, G1c=None, name=''):
    ...
    load_reversal = False
    if not load_reversal:
        w_iter = np.linspace(0.01,w_max,int(w_iter_no_pts))
        # w_iter = np.unique(np.concatenate(( np.linspace(0.01,3.2,int(0.1*w_iter_no_pts)), np.linspace(3.2,3.39,int(0.4*w_iter_no_pts)), 
        #                                 np.linspace(3.4,6,int(0.25*w_iter_no_pts)), np.linspace(6,8,int(0.25*w_iter_no_pts))  )))
    # w_iter = np.linspace(0.01,8,100)
    else:
...
if __name__ == "__main__":
    ...
    # Single run tests
    if local_run:
        if False: # pristine with 1 panel
        # p3_70_25_m18_8_ki1e5_tauo67_nx60_ny30_wpts30_G1c112
            dmg_index, del_d, kw_tsl, force_intgn, c_all = test_dcb_damage_pristine(phy_dim=[3,65,25,0], 
                no_terms=[10,10,8,8], name=['pristine','pristine'], k_i=1e4, tau_o=87, no_x_gauss=50, 
                no_y_gauss=25, w_iter_info=[50,2], G1c=1.12)
            
        if True:
            dmg_index, del_d, kw_tsl, force_intgn, c_all = test_dcb_damage_pristine_mult_pan(phy_dim=[3,65.03,25,0.03], 
                no_terms=[10,10,8,8], name=['pristine_3p',''], k_i=1e4, tau_o=87, no_x_gauss=60, 
                no_y_gauss=30, w_iter_info=[50,1.5], G1c=1.12)