...
def test_dcb_bending_pd_tsl():
    ...
    nr_pan = 2 # no of panels per structure
    print('No of panels = ', nr_pan)
...
def test_junk():
    ...
    if True:
        vec = 'Fxx'
        res_bot = assy.calc_results(c=c0, group='bot', vec=vec, nr_x_gauss=5, y_cte_force=bot2.b)
        print(res_bot)
...
def test_dcb_vs_fem():
    for a2 in [0.15, 0.1,0.099,0.09,0.05,0.01,0.005,0.0015]:
        dcb_vs_fem(nr_pan=3, nr_terms=6, plies=1, disp_mag=0.01, a2=a2, nr_y_gauss=80, grid_x=50)


def dcb_vs_fem(nr_pan, nr_terms, plies, disp_mag, a2, nr_y_gauss, grid_x, kw=None):
    r"""
        Full DCB
    ...
    print(f'a2 = {a2}')
    # nr_pan = 3 # no of panels per structure
    # print('No of panels = ', nr_pan)
...
def single_panel_bending(nr_pan, nr_terms, plies, disp_mag, a1, nr_y_gauss, grid_x):
...
def dcb_one_and_half(nr_pan, nr_terms, plies, disp_mag, a2, nr_y_gauss, grid_x, kw):
...
def tobedeleted_test_leg_sigm(res_x_prev=None, del_d_fit=None):
    import scipy
...
if __name__ == "__main__":
    ...
    # DCB TEST
    # Qxx_end_15_9, dy_Qxx_15_9, Mxx_end_15_9, dy_Mxx_15_9 = test_dcb_vs_fem(nr_pan=3, nr_terms=30, plies=1, disp_mag=1,
    ...
    if False:
        ...
        for a1 in [5]:#,10,17.5,25,32.5,50,62.5,75,82.5,90,95]:
            print(f'a1 {a1}')
            sp_kr7_t8[count, 1] = single_panel_bending(nr_pan=2, nr_terms=8,
                                            plies=1, disp_mag=15, a1=a1, nr_y_gauss=100, grid_x=500)
            sp_kr7_t8[count, 0] = a1
            count += 1
    if False:
        f_calc = single_panel_bending(nr_pan=2, nr_terms=10,
    ...
    if False:
        ...
        for a2 in [0.5]:#,1,5]:#,10,20,30]:
            print(f'a2 {a2}')
            hp_kr9_ksb9_t15[count, 1] = dcb_one_and_half(nr_pan=3, nr_terms=25, plies=1, disp_mag=15,
                                    a2 = a2, nr_y_gauss=300, grid_x=1000, kw=1e5)
            hp_kr9_ksb9_t15[count, 0] = a2
            count += 1
    if False:
        dcb_one_and_half(nr_pan=3, nr_terms=25, plies=1, disp_mag=1075.01,
    ...
    if False:
        ...
        for terms in [4,8,12,15,20,25,30]:#,10,20,30]:
            print(f'terms {terms}')
            hp_kr9_ksb9_t15[count, 1] = dcb_one_and_half(nr_pan=3, nr_terms=terms, plies=1, disp_mag=15,
                                    a2 = 1, nr_y_gauss=300, grid_x=1000, kw=1e5)
            hp_kr9_ksb9_t15[count, 0] = a2
            count += 1

    # monoton_top, monoton_bot = dcb_one_and_half(nr_pan=3, nr_terms=25, plies=1, disp_mag=15,
                            # a2 = 0.5, nr_y_gauss=300, grid_x=1000, kw=1e5)

    # Fitting arbitary curves using different sets of shape functions
    if False:
        # res_x, ier = tobedeleted_test_leg_sigm(res_x_prev=None, del_d_fit=del_d_fit_interp)
